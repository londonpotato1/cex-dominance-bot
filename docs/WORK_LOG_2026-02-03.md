...
## Log Entry – 2026-02-04 12:02
- Summary: BIRB(Upbit/Bithumb) 테스트 상장 데이터 수동 삽입, notice_events/listing_events 증가 확인.
- Files touched:
  - `scripts/insert_manual_listing.py`
  - `data/cex_listing.db`
- Next steps:
  - 실제 상장 시간대 기준으로 outcome tracker 검증

---

## Log Entry – 2026-02-04 12:02 (follow-up)
- Summary: BIRB 상장 공지 기반 테스트 입력 완료 사실 공유 및 자동 기록 파이프라인 연결 안내.
- Files touched:
  - `docs/WORK_LOG_2026-02-03.md`
- Next steps:
  - BIRB 실제 가격/입금/시총 자동 수집 파이프라인 연결 여부 결정

---

## Log Entry – 2026-02-04 12:26
- Summary: mock BTC 상장 이벤트로 펌핑/입금/시총 자동 기록 파이프라인 테스트 성공.
- Files touched:
  - `collectors/storage.py`
  - `scripts/listing_outcome_tracker.py`
  - `scripts/insert_wallet_flow.py`
  - `data/cex_listing.db`
- Next steps:
  - 실제 상장 이벤트(BIRB 등)로 tracker 운영 검증
  - wallet_flows 실수집 연동

---

## Log Entry – 2026-02-04 12:47
- Summary: wallet_flows 실수집 러너 추가, listing_outcomes → CSV 동기화 스크립트 추가, live pipeline 러너 추가 및 실행 테스트.
- Files touched:
  - `scripts/wallet_flow_collector_runner.py`
  - `scripts/sync_outcomes_to_datasets.py`
  - `scripts/run_live_listing_pipeline.py`
  - `scripts/listing_outcome_tracker.py`
- Next steps:
  - ALCHEMY_API_KEY 설정 후 wallet_flow 실수집 확인
  - 실제 상장 이벤트 기반 자동 기록 검증

## Log Entry 2026-02-04 13:00
- Summary: Ran notice collector for 30s; notice/listing counts unchanged due to dedup (notice_events=26, listing_events=6).
- Files touched:
  - data/cex_listing.db
- Next steps:
  - Run wallet flow collector with chain filter + Alchemy key.

## Log Entry 2026-02-04 13:02
- Summary: Added wallet chain filter support (HotWalletTracker + runners) and ran wallet_flow_collector_runner with Ethereum only; no new wallet_flows inserted (count=3).
- Files touched:
  ...
  - data/cex_listing.db
- Next steps:
  - Run live pipeline (notice + wallet + outcome) with Ethereum chain filter.

## Log Entry 2026-02-04 13:04
- Summary: Ran live pipeline for 60s with Ethereum-only wallet tracking; market_snapshots increased (20 total), listing_outcomes unchanged (1). Synced outcomes to datasets (no new rows).
- Files touched:
  - data/cex_listing.db
- Next steps:
  - Keep pipeline running longer to accumulate listing outcomes for new events.

## Log Entry 2026-02-04 13:09
- Summary: Started live listing pipeline (duration=1800s, wallet_chains=ethereum) in background.
- Files touched:
  - logs/live_pipeline_20260204_130806.out.log
  - logs/live_pipeline_20260204_130806.err.log
- Next steps:
  - Let pipeline run to completion; check logs and DB counts.

## Log Entry 2026-02-04 13:09
- Summary: Started wallet flow collector (duration=1800s, chains=ethereum) in background.
- Files touched:
  - logs/wallet_flow_20260204_130832.out.log
  - logs/wallet_flow_20260204_130832.err.log
- Next steps:
  - Let collector run to completion; check wallet_flows for new rows.

## Log Entry 2026-02-04 13:09
- Summary: DB snapshot after starting background jobs: notice_events=26, listing_events=6, wallet_flows=3, market_snapshots=28, listing_outcomes=1.
- Files touched:
  - data/cex_listing.db
- Next steps:
  - Recheck counts after jobs finish; consider longer run if no new deposits.

## Log Entry 2026-02-04 14:23
- Summary: Live pipeline + wallet flow collector completed (30m). market_snapshots grew to 372; no new notices/listings/wallet_flows; outcomes sync produced 0 updates.
- Files touched:
  - data/cex_listing.db
  - logs/live_pipeline_20260204_130806.out.log
  - logs/live_pipeline_20260204_130806.err.log
  ...
  - logs/wallet_flow_20260204_130832.err.log
- Next steps:
  - Keep pipeline running longer or lower min-usd to capture deposits; add more listing events/exchanges.

## Log Entry 2026-02-04 14:33
- Summary: Started wallet flow collector for 1h (min_usd=10000, chain=ethereum) in background.
- Files touched:
  - logs/wallet_flow_20260204_143317.out.log
  - logs/wallet_flow_20260204_143317.err.log
- Next steps:
  - Let collector finish; review wallet_flows.

## Log Entry 2026-02-04 14:35
- Summary: Added Binance polling to live pipeline (global listing source expansion) and ran 20s smoke test.
- Files touched:
  - scripts/run_live_listing_pipeline.py
  - data/cex_listing.db
- Next steps:
  - Run live pipeline with binance_interval in production (set ALCHEMY_API_KEY for wallet tracking).

## Log Entry 2026-02-04 14:37
- Summary: Added premium/funding/DEX snapshot collector and inserted sample snapshots (funding_rates=8, dex_liquidity_snapshots=3).
- Files touched:
  - collectors/storage.py
  - scripts/market_snapshot_collector_runner.py
  - data/cex_listing.db
- Next steps:
  - Run collector on schedule (symbols list + interval) and wire into live pipeline if needed.
